...
//!   * a local managed `hsrd` (full read+write, lifecycle-managed by the app)
//!   * a user-controlled remote `hsrd` (read+write only after explicit trust)
//!   * an external read-only explorer (initially HNSFans) used as a fallback
//!     or as the sole source in `external_read_only` mode.
//!
//! Reads may come from a local/remote `hsrd` or an external read-only explorer.
