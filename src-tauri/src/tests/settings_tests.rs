...
fn test_bulk_update_multiple_assets() {
    ...
    let updated =
        crate::db::queries::bulk_update_status(&conn, &ids, "waiting_transfer_tx").unwrap();
    assert_eq!(updated, 3);
    let assets =
        crate::db::queries::list_assets(&conn, Some("waiting_transfer_tx"), None, None, None, None)
            .unwrap();
...
fn test_wallet_snapshots_list() {
    let conn = setup_db();
    for i in 0..5 {
        crate::db::queries::insert_wallet_snapshot(&conn, "primary", i * 1000, None, i, None)
            .unwrap();
    }
...
fn test_audit_log_entries() {
    let conn = setup_db();
    conn.execute(
        "INSERT INTO audit_log (action, detail) VALUES ('test', 'detail1')",
        [],
    )
    .unwrap();
    conn.execute(
        "INSERT INTO audit_log (action, detail) VALUES ('test', 'detail2')",
        [],
    )
    .unwrap();
    conn.execute(
        "INSERT INTO audit_log (action, detail) VALUES ('other', 'detail3')",
        [],
    )
    .unwrap();
