use crate::commands::csv::{infer_status, normalize_tld, parse_boolish};
...
fn test_parse_boolish_various_true() {
    for val in &[
        "true", "TRUE", "True", "1", "yes", "YES", "y", "Y", "staked", "STAKED",
    ] {
...
fn test_parse_boolish_various_false() {
    for val in &[
        "false", "FALSE", "0", "no", "NO", "", "n", "unstaked", "maybe",
    ] {
...
fn test_infer_status_staked_overrides_hint() {
    assert_eq!(
        infer_status(true, Some("finalized_owned")),
        "do_not_touch_staked"
    );
    assert_eq!(
        infer_status(true, Some("not_started")),
        "do_not_touch_staked"
    );
...
fn test_infer_status_unstaked_with_valid_hints() {
    ...
    assert_eq!(
        infer_status(false, Some("namebase_transfer_requested")),
        "namebase_transfer_requested"
    );
    assert_eq!(
        infer_status(false, Some("waiting_transfer_tx")),
        "waiting_transfer_tx"
    );
    assert_eq!(
        infer_status(false, Some("transfer_seen_on_chain")),
        "transfer_seen_on_chain"
    );
    assert_eq!(
        infer_status(false, Some("waiting_finalize")),
        "waiting_finalize"
    );
    assert_eq!(
        infer_status(false, Some("finalized_owned")),
        "finalized_owned"
    );
    assert_eq!(
        infer_status(false, Some("failed_or_stuck")),
        "failed_or_stuck"
    );
}
