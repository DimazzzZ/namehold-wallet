...
#[test]
fn test_hsrd_balance_deserialization_full() {
...
#[test]
fn test_hsrd_balance_deserialization_minimal() {
...
#[test]
fn test_hsrd_name_deserialization_full() {
    let json = r#"{
        "name": "example",
        "nameHash": "abc123",
        "state": "CLOSED",
...
#[test]
fn test_hsrd_name_deserialization_minimal() {
...
#[test]
fn test_hsrd_wallet_info_deserialization() {
    let json = r#"{
        "wid": 0,
        "id": "primary",
        "network": "main",
...
#[test]
fn test_hsrd_address_deserialization() {
    let json = r#"{
        "name": "default",
        "account": 0,
        "branch": 0,
...
#[test]
fn test_hsrd_owner_deserialization() {
    let json = r#"{"hash": "deadbeef01234567890", "index": 42}"#;
    let owner: ChainOwner = serde_json::from_str(json).unwrap();
    assert_eq!(owner.hash, "deadbeef01234567890");
    assert_eq!(owner.index, 42);
}

#[test]
fn test_hsrd_name_stats_deserialization() {
    let json = r#"{
    ...
    let stats: ChainNameStats = serde_json::from_str(json).unwrap();
...
#[test]
fn test_hsrd_node_info_deserialization() {
    let json = r#"{"version": "7.0.0", "network": "main", "chain": {"height": 100000}}"#;
    let info: ChainNodeInfo = serde_json::from_str(json).unwrap();
    assert_eq!(info.version.as_deref(), Some("7.0.0"));
    assert_eq!(info.network.as_deref(), Some("main"));
}

#[test]
fn test_hsrd_name_with_all_none_fields() {
