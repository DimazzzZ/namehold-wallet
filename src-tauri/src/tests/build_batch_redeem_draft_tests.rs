//! Tests for `build_batch_redeem_draft_inner` — the pure inner logic of the
//! `build_batch_redeem_draft` Tauri command.
//!
//! The inner takes a pre-resolved `Vec<(name, name_hash, bid_commitment,
//! reveal_coin, state)>` (the wrapper's per-name `fetch_name_state` +
//! bid-commitment + losing-reveal-coin lookup) and builds one REDEEM covenant
//! output per name spending the reveal coin.

use std::collections::HashMap;

use crate::commands::names::{build_batch_redeem_draft_inner, Ctx, NameState};
use crate::db;
...
fn reveal_coin(name: &str, txid: &str, addr: &str, value: u64) -> NameCoin {
    let nh_hex = hex::encode(names::hash_name(name).unwrap());
    let cov_json = format!(r#"{{"type":{},"items":["{}"]}}"#, sync::COV_REVEAL, nh_hex);
    NameCoin {
        txid: txid.into(),
...
fn bid_row(name: &str, addr: &str) -> BidCommitmentRow {
    BidCommitmentRow {
        name: name.into(),
...
fn setup(names_in: &[&str]) -> (rusqlite::Connection, Ctx, PerName) {
    let conn = test_db();
    seed_profile(&conn);
    ...
    for (i, name) in names_in.iter().enumerate() {
        ...
        seed_reveal_coin(&conn, name, &txid, 2_000_000, &recv0.address);
        let nh = names::hash_name(name).unwrap();
        per_name.push((
            name.to_string(),
            nh,
            bid_row(name, &recv0.address),
            reveal_coin(name, &txid, &recv0.address, 2_000_000),
            name_state(),
        ));
    }
...
#[test]
fn batch_redeem_happy_path_persists_draft() {
    let (conn, ctx, per_name) = setup(&["alpha", "bravo"]);

    let summary = build_batch_redeem_draft_inner(&conn, &ctx, per_name, 10).unwrap();
...
#[test]
fn batch_redeem_empty_persists_zero_input_draft() {
    ...
    let err = build_batch_redeem_draft_inner(&conn, &ctx, Vec::new(), 10).unwrap_err();
    match err {
...
#[test]
fn batch_redeem_single_name() {
    let (conn, ctx, per_name) = setup(&["solo"]);
    let summary = build_batch_redeem_draft_inner(&conn, &ctx, per_name, 42).unwrap();
    assert_eq!(summary.action, "batch-redeem");
}
