...
fn state_no_owner(
    ...
    derive_auction_task_state(phase, owns_name, has_bid, has_bid_coin, has_reveal, owns_name, None, None)
}
...
fn state_registered(
    ...
    derive_auction_task_state(phase, owns_name, has_bid, has_bid_coin, has_reveal, owns_name, Some(6), None)
}
...
fn state_unregistered(
    ...
    derive_auction_task_state(phase, owns_name, has_bid, has_bid_coin, has_reveal, owns_name, Some(4), None)
}
...
fn state_registered_days(phase: &str, days: Option<f64>) -> AuctionTaskState {
    derive_auction_task_state(phase, true, false, false, false, true, Some(6), days)
}
...
fn explorer_owned_without_owner_coin_within_threshold_yields_expiring_soon() {
    ...
    // loses the name.
    let state = derive_auction_task_state("CLOSED", true, false, false, false, false, None, Some(5.0));
    assert_eq!(state, AuctionTaskState::ExpiringSoon);
}
...
fn won_unregistered_within_threshold_still_needs_register_first() {
    ...
    let state = derive_auction_task_state("CLOSED", true, false, false, false, true, Some(4), Some(5.0));
    assert_eq!(state, AuctionTaskState::WonNeedsRegister);
}
...
fn unowned_closed_within_threshold_is_not_expiring_soon() {
    ...
    let state = derive_auction_task_state("CLOSED", false, false, false, false, false, None, Some(5.0));
    assert_eq!(state, AuctionTaskState::OwnedNoUrgentAction);
}
