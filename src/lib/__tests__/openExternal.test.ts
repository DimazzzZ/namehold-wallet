...
describe("resolveReleaseNotesHref", () => {
  ...
  it("uses HEAD when no tag is provided", () => {
    expect(resolveReleaseNotesHref("docs/X.md")).toBe(`${GITHUB_REPO_URL}/blob/HEAD/docs/X.md`);
  });
  ...
  it("leaves mailto: links untouched", () => {
    expect(resolveReleaseNotesHref("mailto:a@b.com", "v0.6.0")).toBe("mailto:a@b.com");
  });
  ...
  it("leaves protocol-relative URLs untouched", () => {
    expect(resolveReleaseNotesHref("//example.com/x", "v0.6.0")).toBe("//example.com/x");
  });
