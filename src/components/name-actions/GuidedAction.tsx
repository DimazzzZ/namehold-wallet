...
export function GuidedAction({
  ...
  const content = ((): ReactNode => {
    ...
    switch (badge.phase) {
      ...
      case "BIDDING":
        // A wallet that has already bid is still invited to bid again: several
        // independent bids on one name are allowed for the rest of the window.
        // This case used to collapse to a wait-for-reveal panel once a bid was
        // placed, keyed on the backend reporting `waitingForBidding` during
        // BIDDING — which it no longer does, and the panel's copy still named
        // the one-bid-per-wallet rule that was removed with it. How many bids
        // are yours is said by the modal header and the tinted rows in the bid
        // list instead.
        return (
          <BidForm
            variant="guided"
