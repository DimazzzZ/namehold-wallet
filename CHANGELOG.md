...
### Fixed
- **Clicking a button inside an Owned Names row no longer opens two dialogs.** The whole row is clickable and its cells hold their own buttons, so pressing a block height ran the cell's handler and then the row's as the click bubbled up — two dialogs stacked on each other. Ticking the row's select box opened the name dialog for the same reason. A row now ignores clicks that came from a control which handles them itself, decided once at the row rather than by remembering to stop the event in every button. The same rule is applied to the shared `DataTable`, which had the hazard latent.
